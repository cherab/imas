...
def load_wall_mesh(
    ...
    """Loads machine wall components from IMAS wall IDS and creates a dictionary of Raysect mesh
    primitives.
    ...
    :param subsets: A list of names of specific ggd subsets to load. Default is None (loads all
        subsets).
    :param materials: Optional dictionary with Raysect materials for each wall component. Default is
        None. Use component names as keys. The components are splitted by their grid subsets and for
        ...
        "{grid_name}.{subset_name}.{material_name}" E.g.: "TokamakWall.full_main_chamber_wall.Be".
...
def load_wall_outline(
    shot, run, user, database, backend=imas.imasdef.MDSPLUS_BACKEND, occurrence=0, desc_index=0
):
    ...
    description2d = entry.partial_get(
        "wall", f"description_2d({desc_index})", occurrence=occurrence
    )
